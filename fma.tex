...
\begin{table*}[t]
	...
	\begin{tablenotes}
		...
		\item[2] Average duration and sample rate. Most clips from \href{https://www.7digital.com}{7digital} are 30 or 60s and 22 or 44kHz \cite{msd_features}.
...
GTZAN \cite{gtzan}, a collection of 1000 clips from 10 genres, was the first publicly available benchmark MGR dataset. As a result, despites its flaws, it continues to be the most used dataset for MGR \cite{mgr_eval_2}. In addition to its small size, the main problems of GTZAN is mislabelling, repetitions, and distortions, as well as missing metadata (no artist name and song title) which prevent researchers to control for artists or album effects. % Each song is represented by a 22,050Hz Mono 16-bit wav audio file of 30sec.
Looking at \tabref{tab:datasets}, the well-known MagnaTagATune \cite{magnatagatune} and the Million Song Dataset (MSD) \cite{msd} as well as the newer AudioSet \cite{audioset} and AcousticBrainz \cite{acousticbrainz} appear as contenders for a large-scale reference dataset. % They however each have limitations.
...
% Value proposition.
\textbf{Avoiding biases.} sampling bias TODO: reviewer 1 comment 15 \cite{mgr_eval_2}
...
Moreover, researchers are wary of proprietary features such as those computed by commercial services like \href{http://the.echonest.com/}{Echonest}. They have e.g.\ extracted features by themselves on the MSD by downloading sample audio, a tedious process \cite{msd_features}.
...
\textbf{Metadata rich.} \todo{redundant with content} In addition to audio, the dataset comes with a wealth of metadata, shown in \tabref{tab:metadata}. All the metadata available through the API has been archived. In addition to the basics like title, album or artist names, this includes per-track genres and user data such as per-track/album/artist favorites, play counts and comments.
% which are necessary to evaluate content-based recommender systems.
There is also free-form text such as per-track/album/artist tags, album description and artist biography. To compare, the MSD only provides artist-level metadata and tags \cite{msd}, while GTZAN offers none at all.
...
The website provides a large catalog of artists and high-quality tracks, hand-picked by established audio curators. Each track is legally free to download as artists decided to release their works under a permissive license.
...
While root genre recognition should be treated as a multi-label problem in general, \figref{fig:genres_per_track} shows that half the tracks have a single root genre.
...
\tabref{tab:features} lists the features we extracted from the audio with the \href{https://github.com/librosa/librosa}{librosa} Python library, version 0.5.0 \cite{librosa}. These are all the features the library is able to extract.
...
\begin{enumerate}
	...
	\item \textbf{Medium}: 25,000 of the most popular 30s clips with good metadata and quality audio. Genre unbalanced with 21 to 7,103 clips per top genre (\figref{fig:genre_top_distribution}), but only one of the 16 top genres per clip.
		\todo{This subset was constructed for single root genre prediction. The clips were selected based on ... Non-musical genres, experimental?}
...
\end{enumerate}
The above constraints are satisfied for all subsets, and a track is assigned to the same split across all of them.\footref{creation} Each root genre is guaranteed to be represented in all splits, but the ratio is only exact for the small subset (800/100/100). The seven smallest sub-genres (less than 20 tracks) are however not guaranteed to appear in all splits of the full and large sets.
...
While our intention was to release a large volume of audio for machine learning algorithms, analyzing audio is certainly of interest to musicologists and researchers who want to study relations with higher-level representations. Moreover, the metadata is surely a valuable addition to existing datasets\footnote{Like MusicBrainz, AllMusic, Discogs and Last.fm.} for metadata analysis.
...
\subsection{Music Recommendation} % Recommendation, Recommender Systems
% Cite Kirell's paper: Song recommendation with non-negative matrix factorization and graph total variation
%Cheyenne: We do have lists for each user of which songs, albums and artists a user marked as 'favorites' though.
%friends form a social networks
% fans and favorites
% user mixes
...
While the present dump only captures listening and downloading counts in aggregates (\figref{fig:listens_distribution})\footnote{That information can be useful to e.g.\ predict hits.}, the lists of which songs, albums and artists a user marked as favorites or commented are public. While not public, listening and downloading activities are logged and might be shared after anonymization.\footnote{Private discussion with the administrators.} Moreover, users form a public social network via \textit{friend requests}.
