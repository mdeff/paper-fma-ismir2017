...
\begin{table}
	...
	\begin{tabular}{|llll|}
		...
		\hline
	\end{tabular}
	\caption{List of per-track, per-album and per-artist metadata distributed in \texttt{tracks.csv}.}
...
\begin{table*}
	\tiny
	\centering
\begin{tabular}{@{}l@{\tspace}l@{\tspace}l@{\tspace}l@{\tspace}r@{\tspace}r@{\tspace}l@{\tspace}r@{\tspace}l@{\tspace}l@{\tspace}l@{}}
	\toprule
	...
	\bottomrule
	\end{tabular}
	\caption{Some rows and columns of the table stored in \texttt{tracks.csv}.}
	\label{tab:tracks}
\end{table*}
...
\begin{table}
	...
	\begin{tabular}{ccclr}
		...
		\bottomrule
	\end{tabular}
	\caption{An excerpt of the genre hierarchy stored in \texttt{genres.csv}. Top part is some of the 16 top-level genres, bottom part is some second- and third-level genres.}
	\label{tab:genres}
\end{table}
...
\begin{figure}
	...
	\caption{Power law distribution of the genres.}
...
\begin{figure}
	...
	\caption{Example of genre hierarchy for the top-level Soul-RnB genre. Left number is the \texttt{genre\_id}, right number is the number of tracks.}
...
\begin{enumerate}
	\item \textbf{Full}: the complete dataset, described above.
	...
	\item \textbf{Medium}: 25,000 popular tracks with good metadata, e.g. only one top-level genre, and quality audio.
	\item \textbf{Small}: 8,000 tracks from 8 top-level genres, balanced with 1000 tracks per genre, 1 top genre per track. This subset is similar to the very popular GTZAN \cite{gtzan} with the added benefits of the FMA, that is metadata, pre-computed features and copyright-free audio.
\end{enumerate}
...
\begin{enumerate}
	...
	\item The three sets were forced to have the same distribution of songs per artist. %The three sets were alternatively filled in descending order of the count of artist songs. 
\end{enumerate}
