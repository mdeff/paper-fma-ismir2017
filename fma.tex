...
It features a collection of meta-data such as the song title, artist name, album or genres, together with full-length and high-quality audio, user data such as play counts, as well as a set of pre-computed features. We propose a train/validation/test split and three subsets: a small genre-balanced dataset of 4,000 songs and 10 genres, a medium genre-unbalanced dataset of 14,511 songs and 20 genres, and a 90 GiB version of the full set, with clips trimmed to 30s.
...
\begin{table}
	...
	\begin{tabular}{|llll|}
		\hline
		track\_id & track\_title & album\_title & artist\_name \\
		genres & play\_count & & \\
		\hline
	\end{tabular}
...
A split into training, validation and test sets is proposed to make research on the FMA reproducible. The training and validation sets should be merged if cross-validation is used instead.
Below are the constraints we followed:
\begin{enumerate}
	\item The songs were split with the same ratio 80/10/10\% across all genres, i.e. a ratio of about 6 between training and validation/test sets.
	% PTB 19/3/3, MNIST 60/10, ImageNet 1200/50/100 --> ratio ~6-12
	\item We forced an artist to appear in one set only, i.e. all songs from a given artist either appear in test, validation or train.
	\item We alternatively filled the three sets in descending order of the count of artist songs. It forces the three sets to have the same distribution of songs per artist.
\end{enumerate}
...
% Graph ConvNet
% Would be good to popularize the method and make the connection with my core research. Will have to be compared to std ConvNets.
% Node classification: each node is a track
%	* powerful in a semi-supervised setting (traditional use of graphs in ML)
%	* do this with Chibu or as a master thesis
%	* artist relations --> disconnected graph
%	* user relations (users playlists form communities)
% Kirell [ICASSP recommender avec Vassilis]:
%	* généralement c'est la similarité musicale
%	* mix: metadata + audio features + social
%	* on apprend une métrique qui pondère bien le tout par rapport au genre par exemple
% Signal classification would mean finding a representation which can be supported by a graph. Notes on spectrograms ?
...
% Moreover, it is one of the most researched areas of MIR.
