...
It provides metadata like song title, album, artist and genres; user data like play counts, favorites, and comments; free-form text like description, biography, and tags; together with full-length, high-quality audio, and some pre-computed features. We propose a train/validation/test split and three subsets: a genre-balanced set of 8,000 tracks from 8 major genres, a genre-unbalanced set of 25,000 tracks from 16 genres, and a 98 GiB version with 30s clips.
...
Looking at \tabref{tab:datasets}, the well-known MagnaTagATune \cite{magnatagatune} and the Million Song Dataset (MSD) \cite{msd} as well as the newer AudioSet \cite{audioset} appear as contenders for a large-scale reference dataset. % They however each have limitations.
...
MSD and AudioSet, while legal and very large, force researchers to download audio clips from online services.
...
\begin{figure}
	...
	\caption{Track duration, from 0 to 3 hours (zoomed).}
...
\begin{figure}
	...
	\caption{Album listens, from 0 to 3.6 millions (zoomed).}
...
\figref{fig:genres_per_track}~and~\ref{fig:genre_distribution} shows the number of genres per track and tracks per genre. As often encountered in natural categories, it obeys a power law.
...
\begin{enumerate}
	...
	\item \textbf{Medium}: 25,000 of the most popular tracks with good metadata and quality audio. Genre unbalanced with 21 to 7,103 tracks per top genre (\figref{fig:genre_top_distribution}), but only one top genre per track.
		\todo{Purpose: single genre prediction.}
...
\begin{enumerate}
	...
	\item An artist filter for artists to be part of one set only. It has been shown that the use of songs from the same artist in both training and test sets leads to over-optimistic accuracy and may favor some approaches \cite{artist_filters_1, artist_filters_2}.
%All songs from a given artist either appear in train, validation or test, never in two of them.
	...
	\item \todo{Artists with lots of tracks were allocated across sets, i.e.\ the three sets were alternatively filled in descending order of the count of artist songs.}
...
The FMA is especially suited for MGR as it features fine genre information, i.e.\ multiple (sub-)genres associated to individual songs, has a built-in genre hierarchy (\tabref{tab:genres}), and is annotated by the artists themselves. While the artists are the best placed to judge the positioning of their creations, they might be inconsistent and motivated by factors not necessarily objective, such as achieving a higher play count.\footnote{While the content is curated, the curators focus on the musical content not the metadata.} Ideally those labels should be one of many ground truths, to be complemented by crowd-sourcing and experts (from different music metadata websites).
...
While not scraped in the present dump, the lists of which songs, albums and artists a user marked as favorites or commented are public. Listening and downloading counts are however only available in aggregates\footnote{That information can still be useful to e.g.\ predict hits.} (\figref{fig:listens_distribution}) as most of that activity happens by users that are not logged in. Moreover, users can send \textit{friend requests} to each other forming a social network.
