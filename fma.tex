...
% about 150-200 words.
\begin{abstract}
We introduce the Free Music Archive (FMA), an open and easily accessible dataset which can be used to evaluate several tasks in music information retrieval (MIR) such as music classification and annotation or recommendation.
...
It features a collection of metadata such as the song title, album, artist name and genres; user data such as play counts, favorites, and comments; free-form text such as description, biography, and tags; together with full-length and high-quality audio as well as some pre-computed features. We propose a train/validation/test split and three subsets: a small genre-balanced set of 8,000 tracks from 8 major genres, a medium genre-unbalanced set of 25,000 tracks from 16 genres, and a 94 GiB version of the full set with clips trimmed to 30s.
...
\tabref{tab:features} (upper part) lists the features we extracted from the audio with the \href{https://github.com/librosa/librosa}{librosa} Python library, version 0.5.0 \cite{librosa}.
...
A more thorough investigation of the available tags may reveal the feasibility of mood classification or instrument recognition. Similarly, cover song detection may be doable if multiple versions of many songs are featured.
...
% style?
% Music classification
	% mood classification --> may be in tags
