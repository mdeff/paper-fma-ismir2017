...
\todo{For statistical learning algorithms, the scale of FMA is comparable to ImageNet and the most used ILSVRC (\tabref{tab:size}). % and shall thus be sufficient for the foreseable future.
...
\tabref{tab:datasets} shows that while the smaller datasets are usually distributed with audio, most of the larger do not.
% often regardless of copyrights
...
Moreover, researchers should be wary of proprietary features such as those computed by commercial services like \href{http://the.echonest.com/}{Echonest}. % They have e.g.\ extracted features by themselves on the MSD by downloading sample audio, a tedious process \cite{msd_features}.
...
\textbf{Metadata rich.} In addition to audio, the dataset comes with rich metadata, shown in \tabref{tab:metadata}. While not complete, it compares favorably with the MSD which only provides artist-level metadata \cite{msd} or GTZAN which offers none.
...
Diversity is another issue: as suggested by \figref{fig:genre_distribution}, this collection is biased toward experimental, electronic, and rock music. Moreover, it probably does not contain mainstream music.
%hits from renown artists or commercial music.
